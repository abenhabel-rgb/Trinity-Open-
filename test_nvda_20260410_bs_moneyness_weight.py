#!/usr/bin/env python3
"""Diagnostic replication: test whether the GEX->VEX transform is explained by
Black-Scholes vanna/gamma moneyness weight on NVDA 2026-04-10.

Observed HeatSeeker snapshot:
- published 2026-04-10 14:59 Paris = 08:59 ET
- spot shown on both panels: 184.03
- same-expiry GEX/VEX values transcribed previously for 2026-04-10, 04-13, 04-15
...
no-intercept scale factor per expiry.
...
SYMBOL = "NVDA"
DATE = "20260410"
SNAPSHOT_ET = "08:59:00"
SPOT = 184.03
NY = ZoneInfo("America/New_York")
SNAPSHOT_DT = datetime(2026, 4, 10, 8, 59, tzinfo=NY)
OUT = Path("nvda_20260410_bs_moneyness_weight_result.json")
...
DATA = {
    "20260410": {
        "gex": np.array([-22.6,115.4,1425.6,0,0,860.0,0,0,20292.7,0,0,10096.3,0,0,24814.4,0,0,68835.1,0,0,1168.7,0,0,8946.1,0,0,6655.0]),
        "vex": np.array([-272.9,2711.5,32527.2,0,0,18888.3,0,0,374573.0,0,0,121377.0,0,0,89584.3,0,0,-306918.4,0,0,-12724.7,0,0,-127357.4,0,0,-112342.0]),
    },
    "20260413": {
        "gex": np.array([-68.2,6.9,234.5,0,0,-33.8,0,0,3439.6,0,0,-1579.0,0,0,-6271.1,0,0,-2358.7,0,0,-84.7,0,0,-606.3,0,0,262.8]),
        "vex": np.array([-3103.6,301.7,9166.3,0,0,-1131.3,0,0,86351.4,0,0,-23879.5,0,0,-27253.9,0,0,13594.3,0,0,1022.9,0,0,13436.1,0,0,-7042.4]),
    },
    "20260415": {
        "gex": np.array([44.5,61.0,96.0,0,0,-100.2,0,0,-391.5,0,0,-211.9,0,0,-231.0,0,0,-590.4,0,0,-536.2,0,0,-50.1,0,0,256.3]),
        "vex": np.array([2256.1,2845.5,3783.1,0,0,-3230.5,0,0,-9085.9,0,0,-2864.1,0,0,-923.8,0,0,2960.5,0,0,7055.0,0,0,1023.0,0,0,-7664.5]),
    },
}
...
print("NVDA 2026-04-10 — BLACK-SCHOLES MONEYNESS WEIGHT REPLICATION")
...
for expiration, d in DATA.items():
    ...
    exp_result = {
        ...
        "bs": {"spearman": rho_bs, "pearson": pear_bs, "alpha": a_bs, "alpha_over_spot": a_bs / SPOT, "r2": r2(vv, yhat_bs), "sign_hits": sign_hits},
